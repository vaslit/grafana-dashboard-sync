...
async function activate(context) {
    ...
    async function updateDashboardHeaderDescription() {
        const activeTarget = currentActiveTarget();
        const devTarget = repository ? await repository.getDevTarget().catch(() => undefined) : undefined;
        const devLabel = devTarget?.instanceName && devTarget?.targetName
            ? `${devTarget.instanceName}/${devTarget.targetName}`
            : "none";
        const activeLabel = activeTarget.instanceName && activeTarget.targetName
    ...
    const dashboardsProvider = new dashboardTreeProvider_1.DashboardTreeProvider(() => repository, () => service, currentActiveTarget, missingProjectMessage);
    const instancesProvider = new instanceTreeProvider_1.InstanceTreeProvider(() => repository, currentActiveTarget, missingProjectMessage);
    ...
    const actionHandlers = {
        ...
        openDashboardInBrowser: async (selectorName, instanceName, targetName) => {
            const service = requireService();
            const record = await requireDashboardRecord(selectorName ?? selectionState.selectedDashboardSelectorName);
            const target = await pickRequiredDeploymentTarget(instanceName, targetName);
        ...
        addDashboard: async () => {
            ...
            if (nextAction?.action === "pull") {
                ...
                await refreshAll();
                void vscode.window.showInformationMessage(`Dashboards pulled from dev target ${devTarget.instanceName}/${devTarget.name}: ${summary.updatedCount} updated, ${summary.skippedCount} unchanged.`);
                return;
            }
        ...
        saveDashboardDatasourceMappings: async (instanceName, targetName, selectorName, values) => {
            const service = requireService();
            await service.saveDatasourceSelections(instanceName, targetName, selectorName, datasourceSelectionsFromFormValues(values, "dashboard"));
            await refreshAll();
        ...
        setTargetRevision: async (selectorName, revisionId, instanceName, targetName) => {
            const service = requireService();
            const record = await requireDashboardRecord(selectorName);
        ...
        deleteRevision: async (selectorName, revisionId) => {
            const service = requireService();
            const record = await requireDashboardRecord(selectorName);
        ...
        pullAllDashboards: async (instanceName, targetName) => {
            ...
            await refreshAll();
            void vscode.window.showInformationMessage(`Pull complete from dev target ${target.instanceName}/${target.name}: ${summary.updatedCount} updated, ${summary.skippedCount} unchanged.`);
        },
    ...
    async function requireDevTarget() {
        const repository = requireRepository();
    ...
    async function pickDashboardExecutionScope(record, action) {
        const repository = requireRepository();
    ...
    async function renderDashboardAcrossScope(record) {
        const service = requireService();
    ...
    async function deployDashboardAcrossScope(record) {
        const service = requireService();
    ...
    async function openDashboardInBrowser(item) {
        if (isInstanceTargetDashboardItem(item)) {
            await actionHandlers.openDashboardInBrowser(item.record.selectorName, item.target.instanceName, item.target.name);
            return;
        }
    ...
    async function checkoutRevisionFromTree(item) {
        if (!item) {
    ...
    async function setTargetRevisionFromTree(item) {
        if (!item) {
            throw new Error("Select a dashboard revision first.");
        }
        const target = await pickRequiredDeploymentTarget();
    ...
    async function deployRevisionToActiveTargetFromTree(item) {
        if (!item) {
            throw new Error("Select a dashboard revision first.");
        }
        const target = await pickRequiredDeploymentTarget();
    ...
    async function deleteRevisionFromTree(item) {
        if (!item) {
            throw new Error("Select a dashboard revision first.");
        }
        const confirmed = await vscode.window.showWarningMessage(`Delete revision ${item.revision.record.id} for ${item.record.selectorName}?`, { modal: true }, "Delete");
        if (confirmed !== "Delete") {
            return;
        }
        selectionState.setDashboard(item.record.selectorName);
    ...
    async function pullDashboards(item) {
        ...
        if (isDashboardScopeItem(item)) {
            ...
            await refreshAll();
            void vscode.window.showInformationMessage(`Pull complete for ${item.record.selectorName} from dev target ${devTarget.instanceName}/${devTarget.name}: ${summary.updatedCount} updated, ${summary.skippedCount} unchanged.`);
            return;
        }
        if (isInstanceTargetDashboardItem(item)) {
        ...
        if (item instanceof instanceTreeProvider_1.InstanceTreeItem) {
            ...
            const summary = await service.pullDashboards(entries, devTarget.instanceName, devTarget.name);
            await refreshAll();
            void vscode.window.showInformationMessage(`Pull complete for all dashboards from dev target ${devTarget.instanceName}/${devTarget.name}: ${summary.updatedCount} updated, ${summary.skippedCount} unchanged.`);
            return;
        }
        if (item instanceof instanceTreeProvider_1.DeploymentTargetTreeItem) {
            if (!sameTarget(item.target, devTarget)) {
        ...
        const summary = await service.pullDashboards(entries, devTarget.instanceName, devTarget.name);
        await refreshAll();
        void vscode.window.showInformationMessage(`Pull complete from dev target ${devTarget.instanceName}/${devTarget.name}: ${summary.updatedCount} updated, ${summary.skippedCount} unchanged.`);
    }
