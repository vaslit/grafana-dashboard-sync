...
exports.extractDashboardDatasourceRefs = extractDashboardDatasourceRefs;
exports.buildDashboardDatasourceDescriptors = buildDashboardDatasourceDescriptors;
function isBuiltinGrafanaDatasource(uid, type) {
    return uid === "-- Grafana --" || (uid === "grafana" && (type === "grafana" || type === "datasource"));
}
function registerRef(refs, uid, type, usageKind) {
...
function walk(node, refs) {
    ...
    if (typeof datasource === "string") {
        ...
        const datasourceRecord = datasource;
        if (typeof datasourceRecord.uid === "string") {
            registerRef(refs, datasourceRecord.uid, typeof datasourceRecord.type === "string" ? datasourceRecord.type : undefined, typeof record.refId === "string" ? "query" : "panel");
        }
    }
...
function extractDashboardDatasourceRefs(dashboard) {
    const refs = new Map();
    walk(dashboard, refs);
    return [...refs.values()].sort((left, right) => left.sourceUid.localeCompare(right.sourceUid));
}
function uniqueDatasourceKey(baseLabel, sourceUid, usedKeys) {
...
function buildDashboardDatasourceDescriptors(dashboard, datasources) {
...
//# sourceMappingURL=datasourceRefs.js.map